...
@section('title', 'Modifier Page Accueil')
...
<div class="flex items-center gap-4 mb-8">
    ...
    <div>
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Modifier Page Accueil 🏠</h1>
        <p class="text-slate-500 text-sm mt-1">Gérez les sections, textes et images de la page d'accueil.</p>
    </div>
    <a href="{{ route('public.home') }}" target="_blank" class="ml-auto flex items-center gap-2 px-5 py-2.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 rounded-xl font-bold text-xs uppercase tracking-widest transition-all">
        ...
        <div class="p-6 flex gap-6">
            ...
            <div class="flex-shrink-0">
                ...
                <div class="relative group w-36 h-28 rounded-xl overflow-hidden border border-slate-200 shadow-sm">
                    <img src="{{ asset('storage/' . $item->image) }}" class="w-full h-full object-cover" id="img-preview-{{ $item->id }}">
                    <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center gap-2">
                        ...
                        <button onclick="deleteImage({{ $item->id }}, this)" class="w-8 h-8 bg-red-500 text-white rounded-full flex items-center justify-center text-xs" title="Supprimer">
                            <i class="fa-solid fa-trash"></i>
                        </button>
                    </div>
                </div>
                @else
                <label class="cursor-pointer w-36 h-28 rounded-xl border-2 border-dashed border-slate-200 hover:border-blue-400 flex flex-col items-center justify-center gap-2 text-slate-300 hover:text-blue-400 transition-colors group">
...
<div id="editSectionModal" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center hidden p-4">
    <div class="bg-white rounded-[2.5rem] w-full max-w-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <form action="{{ route('dashboard.admin.pages.section.update') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-5 overflow-y-auto flex-1" id="editForm">
            @csrf
            <input type="hidden" name="id" id="section_id">
            <input type="hidden" name="clear_image" id="clear_image_flag" value="0">

            <div>
                <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Titre / Entête</label>
            ...
            <div>
                ...
                <textarea name="content" id="section_content" rows="5" oninput="updatePreview()"
            ...
            <div>
                ...
                <div id="imageArea">
                    {{-- Existing image --}}
                    <div id="existingImageWrap" class="hidden">
                        <div class="relative group rounded-xl overflow-hidden border border-slate-200 shadow-sm max-w-xs">
                            <img src="" alt="" id="existingImagePreview" class="w-full h-40 object-cover">
                    ...
                    {{-- No image / new upload zone --}}
                    <label id="uploadZone" class="cursor-pointer hidden flex flex-col items-center justify-center border-2 border-dashed border-slate-200 hover:border-blue-400 rounded-xl py-8 gap-3 text-slate-300 hover:text-blue-400 transition-colors group">
                        <i class="fa-solid fa-cloud-arrow-up text-3xl"></i>
                        <span class="text-sm font-bold">Cliquer ou glisser une image ici</span>
                        <span class="text-xs opacity-70">PNG, JPG, WEBP — max 5MB</span>
                        <input type="file" name="image" accept="image/*" class="hidden" onchange="previewNewFile(this)" ondragover="this.parentElement.classList.add('border-blue-400')" ondragleave="this.parentElement.classList.remove('border-blue-400')">
                    </label>
                </div>
            </div>

            {{-- Live Preview --}}
            <div id="livePreviewWrap" class="bg-slate-50 border border-slate-100 rounded-xl p-5">
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Aperçu en direct</p>
...
function deleteImage(pageId, btn) {
...
function openEditModal(item) {
    ...
    const existingWrap = document.getElementById('existingImageWrap');
    const uploadZone   = document.getElementById('uploadZone');
    const existingImg  = document.getElementById('existingImagePreview');

    if (item.image) {
        existingImg.src = '/storage/' + item.image;
...
function clearImage() {
    ...
    document.getElementById('existingImageWrap').classList.add('hidden');
...
function previewNewFile(input) {
    ...
    reader.onload = e => {
        ...
        document.getElementById('existingImageWrap').classList.remove('hidden');
...
function updatePreview() {
    document.getElementById('preview_title').textContent   = document.getElementById('section_title').value;
    document.getElementById('preview_content').textContent = document.getElementById('section_content').value;
}
