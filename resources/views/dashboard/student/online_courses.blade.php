...
@section('content')
    <div class="mb-8">
        ...
        <p class="text-slate-500 text-sm mt-1">Accédez à vos sessions de cours à distance.</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden flex flex-col transition-all hover:shadow-md">
            <div class="p-6 flex-grow">
                ...
                <p class="text-xs font-bold text-blue-600 mb-3 italic">Par {{ $course->professor->name }}</p>
            ...
            <div class="p-4 bg-slate-50 border-t border-slate-100">
                ...
                <button disabled title="Disponible 15 mins avant" class="w-full py-2.5 bg-slate-200 text-slate-400 rounded-xl font-bold text-sm cursor-not-allowed flex items-center justify-center gap-2">
                    <i class="fa-solid fa-lock text-xs"></i> Masqué
                </button>
                <p class="text-[10px] text-center text-slate-400 mt-2 font-bold uppercase tracking-widest text-xs">Ouvre à {{ $startTime->copy()->subMinutes(15)->format('H:i') }}</p>
                @elseif($finished)
        ...
        <div class="col-span-full py-20 text-center text-slate-400 italic bg-white rounded-3xl border border-dashed border-slate-200">
            Aucun cours en ligne planifié pour votre classe.
        </div>
        @endforelse
    </div>
